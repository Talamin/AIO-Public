...
namespace AIO.Combat.Warrior
{
    ...
    internal class GroupProtection : BaseRotation
    {
        ...
        protected override List<RotationStep> Rotation => new List<RotationStep> {
            ...
            new RotationStep(new RotationSpell("Shield Block"), 2.15f, (s,t) => (t.CHealthPercent() > 70 || t.IsElite || t.IsBoss) && EnemiesAttackingGroup.ContainsAtLeast(o => o.CGetDistance() <=10 && o.CIsTargetingMe(), Settings.Current.ProtectionShieldBlock), _ => Me.CHealthPercent() < 90, RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell("Enraged Regeneration"), 2.151f, RotationCombatUtil.Always, _ => Me.CHealthPercent() <= Settings.Current.ProtectionEnragedRegeneration && Me.CHaveBuff("Bloodrage") && Me.CBuffTimeLeft("Bloodrage") <= 2000, RotationCombatUtil.FindMe, checkRange: false),
            new RotationStep(new RotationSpell("Intercept"), 2.16f, (s,t) => Settings.Current.ProtectionIntercept && t.CGetDistance() > 8, RotationCombatUtil.BotTargetFast, checkLoS: true),
            new RotationStep(new RotationSpell("Charge"), 2.17f, (s,t) => Settings.Current.ProtectionIntercept && t.CGetDistance() > 8, RotationCombatUtil.BotTargetFast, checkLoS: true),
            new RotationStep(new RotationSpell("Shield Bash"), 2.17f, (s,t) => t.CCanInterruptCasting(), RotationCombatUtil.BotTargetFast),
            ...
            new RotationStep(new RotationSpell("Taunt"), 4f, (s,t) => !t.CIsTargetingMe(), _ => Settings.Current.ProtectionTauntGroup && Me.CIsInGroup(), FindEnemyAttackingGroup),
            new RotationStep(new RotationSpell("Taunt"), 4.1f, (s,t) => !t.CIsTargetingMe(), _ => !Me.CIsInGroup(), RotationCombatUtil.BotTargetFast),
            ...
            new RotationStep(new RotationSpell("Thunder Clap"), 10f, RotationCombatUtil.Always, _ => EnemiesAttackingGroup.Any(unit => unit.CGetDistance() < 8 && !unit.CHaveMyBuff("Thunder Clap")), RotationCombatUtil.FindMe, checkRange: false),
            ...
            new RotationStep(new RotationSpell("Devastate"), 10.4f, (s,t) => Me.CRage() > 70 || t.CMyBuffStack("Sunder Armor") < 5, RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell("Bloodrage"), 11f, (s,t) => Me.CRage() < 70, RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell("Cleave"), 13f, RotationCombatUtil.Always, _ => Me.CRage() > Settings.Current.ProtectionCleaveRageCount && !Me.CHaveBuff("Glyph of Revenge") && EnemiesAttackingGroup.ContainsAtLeast(o => o.CGetDistance() < 10, Settings.Current.ProtectionCleaveCount), RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell("Concussion Blow"), 15f, (s,t) => t.CHealthPercent()  > 40, RotationCombatUtil.BotTargetFast),
            new RotationStep(new RotationSpell("Demoralizing Shout"), 19f, RotationCombatUtil.Always, _ => EnemiesAttackingGroup.ContainsAtLeast(unit => unit.CGetDistance() < 15 && unit.CHealthPercent() > 65 && !unit.CHaveBuff("Demoralizing Shout") && !unit.CHaveBuff("Demoralizing Roar"), Settings.Current.ProtectionDemoralizingCount), RotationCombatUtil.FindMe, checkRange: false),
